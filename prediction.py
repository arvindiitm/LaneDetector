...
#print("list of images",folder_list)
images = list()
print('[Read images]')
for i, i_f in enumerate(folder_list):
        if i == 1500:
            break
...
original_video_out = cv.VideoWriter('./outputs/seq_original.avi', video_codec, 15, image_size)
overlay_video_out = cv.VideoWriter('./outputs/seq_overlay.avi', video_codec, 15, image_size)
seg_video_out = cv.VideoWriter('./outputs/seq_seg.avi', video_codec, 15, image_size)
...
for i in images:
    ...
    cv.imwrite("./outputs/"+ str(count)+"_overlay.png",cv.cvtColor(overlay, cv.COLOR_RGB2BGR))
    ...
    cv.imwrite("./outputs/"+ str(count)+"_red.png",cv.cvtColor(red_image.astype(np.uint8), cv.COLOR_RGB2BGR))
    timer.tick_timer()
    count+=1
